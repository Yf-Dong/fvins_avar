 ...
 */
void fvins_walltime(uint32_t& sec, uint32_t& nsec)
#ifndef WIN32    
 ...
 */
int fvins_nanosleep(const uint32_t &sec, const uint32_t &nsec) {
#if defined(WIN32)
  HANDLE timer = NULL;
 ...
 */
bool fvins_wallsleep(uint32_t sec, uint32_t nsec) {
#if defined(WIN32)
  fvins_nanosleep(sec,nsec);
#else
  timespec req = { sec, nsec };
  timespec rem = { 0, 0 };
...
bool Time::waitForValid() {
  return waitForValid(Fvins::WallDuration());
}

bool Time::waitForValid(const Fvins::WallDuration& /*timeout*/) {
...
bool Duration::sleep() const {
  if (Time::useSystemTime()) {
    return fvins_wallsleep(sec, nsec);
  } else {
    Time start = Time::now();
    Time end = start + *this;
    if (start.isZero()) {
